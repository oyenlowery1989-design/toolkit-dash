...
- **Attributed person** (group-level, optional): `personName`/`personRole` (e.g. "Jane Doe — CEO") — one combined inline editor (both fields saved together, mirrors the member-edit multi-field pattern rather than one-row-per-field). **Placeholder pending a proper Persons module** — a relational "Important Persons" registry (person ↔ asset groups/addresses/projects, groupable by person or Telegram channel) is planned; these flat fields may be superseded by a foreign key once that module exists — don't build more UI on top of `personName`/`personRole` without checking whether the Persons module has landed first
