...
| `bulk-payments` | Working — secret key field replaced by active wallet indicator when wallet connected; reviewed+fixed 2026-07-06 (fatal-throw false-success, stale exclude-list, abort-on-unmount); reviewed+fixed again 2026-07-13 (custom-asset preflight balance/trustline check, retry-single-batch instead of infinite-loop-on-one-bad-recipient, abort no longer strands batches at "Pending", Save List fixed on From-Group tab) |
| `ghost-payments` | Working — secret key field replaced by active wallet indicator when wallet connected; reviewed+fixed 2026-07-06 (2 CRITICAL: no_trust mode's "no funds will move" guarantee was bypassable, underfunded overshoot only checked sender #1); reviewed+fixed again 2026-07-13 (CRITICAL: underfunded overshoot amount went stale on a mid-preview wallet switch, could move real money; no_trust issuer never checked; trustline_touch preview didn't disclose reserve lock) |
...
| `payments` | Working — Send (multi-leg, Max, remove-trustline + offer cancel), Path (strict-receive + strict-send), Claimable Balance, Fee Bump; ShortAddress on all destinations; reviewed+fixed 2026-07-06 (Max button reserve/liabilities calc, buying-side offer cancellation, stale path selection); reviewed+fixed again 2026-07-13 (19 findings — Max fee/sponsorship accounting, offer-fetch dedup+pagination, path quote staleness, claimant cap/dedupe, fee-bump min-fee check — see `app/(tools)/payments/CLAUDE.md`); layout tightened to fit ~1 screen without scrolling |
...
| `auto-send-groups` | Working — scheduled XLM distribution groups; see full section below; UI fully converted to shared `Button`/`Input`/`Select` (no raw HTML form elements, theme-safe in light mode); reviewed+fixed (2 CRITICAL cross-tenant auth bugs); reviewed+fixed again 2026-07-13 (scheduler was SQLite-only, invisible to Supabase-resident groups in local-Supabase mode; dismiss-banner never persisted; handleDuplicate write-ordering race) |
| `tiered-rewards` | Working — tiered per-holder reward distribution; multi-asset tiers; scheduled or manual; batch/separate mode; JSON import; preview modal; run history; XLM sentinel handled case-insensitively throughout; `dbAction` rolls back optimistic cache + toasts on server rejection; reviewed+fixed 2026-07-06 (2 CRITICAL: Supabase update wiped secret_key/schedule on every patch, no execution-lock risked full double-payment); reviewed+fixed again 2026-07-13 (CRITICAL: Import & Replace tiers deleted existing tiers then silently failed to recreate them; same scheduler Supabase-blind-spot as auto-send-groups; see `components/tiered-rewards/CLAUDE.md`) |
