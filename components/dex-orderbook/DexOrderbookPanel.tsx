...
export function DexOrderbookPanel() {
  const searchParams = useSearchParams();
  const urlBase = searchParams.get("base");
  const urlBaseIssuer = searchParams.get("baseIssuer");

  const [sellCode, setSellCode] = useState(() => urlBase ?? "XLM");
  const [sellIssuer, setSellIssuer] = useState(() => urlBaseIssuer ?? "");
  ...
  function validateAsset(
    ...
    const isXlm = code.toUpperCase() === "XLM";
    if (!isXlm && !ASSET_CODE_RE.test(code.toUpperCase()))
      return `${label} asset code must be 1–12 alphanumeric characters.`;
    if (!isXlm) {
      if (!issuer.trim())
  ...
  // ---------------------------------------------------------------------------
  // Paste handling — CODE:ISSUER pairs or Lobstr trade URLs
  // ---------------------------------------------------------------------------

  const handleSellCodePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    const pasted = e.clipboardData.getData("text");
    const pair = parseAssetPair(pasted);
    if (pair) {
      setSellCode(pair.assetCode);
      setSellIssuer(pair.issuer);
      e.preventDefault();
    }
  };

  const handleBuyCodePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    const pasted = e.clipboardData.getData("text");
    const pair = parseAssetPair(pasted);
    if (pair) {
      setBuyCode(pair.assetCode);
      setBuyIssuer(pair.issuer);
      e.preventDefault();
    }
  };
